...
float cost(vector<vector<float> > &centers)
{
	...
	for(i=0;i<total_data_points;i++)
	{
		...
		for(int j=0;j<centers.size();j++)
		{
            float r=calc_distance(centers[j],input_set[i]);
            //cout << "r= " << r << endl;
...
// }
    ...
    // cout << "size input set= " << input_set.size();
    // for(int i=0;i<input_set.size();i++)
    // {
    //     cout << "ith= " << i << " size=  " << input_set[i].size() << endl;
    // }
    ...
    if (taskid == MASTER)
    {
        ...
        for(int i=1;i<numtasks;i++)
        {
            ...
            cout  << "thread id = " << i <<  " send_size= " << send_size << endl;
            MPI_Send(
                &send_size,
                1,
                MPI_INT,
