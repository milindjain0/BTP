...
void main_openmp(vector<int> *indices,vector<int> index_of_n_sampled_points)
{
	...
	for(int i = 0 ; i < data_size;i++)
	{
		for(int j=0;j<DIM;j++)
		{
			mean[j]+= ((input_set[index_of_n_sampled_points[data.at(i)]][j] *1.0)/(data_size));
		}
	}
	...
	for(int i=1;i<K;i++)
	{
		...
		for(int i = 0 ; i < data_size;i++)
		{
			for(int j=0;j<DIM;j++)
			{
				mean1[j]+= ((input_set[index_of_n_sampled_points[data1.at(i)]][j] *1.0)/(data_size1));
			}
		}
...
int main(int argc, char *argv[])
{
    ...
    if (taskid == MASTER)
    {
        ...
        MPI_Bcast(
            ...
            index_of_n_sampled_points.size(),
            MPI_INT,
            MASTER,
            MPI_COMM_WORLD);
        int i1;
    ...
    else
    {
        MPI_Bcast(
            ...
            MASTER,
            MPI_COMM_WORLD);
        index_of_n_sampled_points.resize(sz);
        MPI_Bcast(
            &index_of_n_sampled_points[0],
            sz,
            MPI_INT,
    ...
    for(int i=0;i<sz_all_subset_point;i++)
    {
        ...
        for(int j=0;j<M;j++)
        {
            tmp.push_back(all_subset_points1[i+j]);
        }
